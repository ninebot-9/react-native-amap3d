...
class Marker: UIView {
    ...
    @objc func setTrueHeading(_ trueHeading: CGFloat) {
        UIView.animate(withDuration: 0.1) {
            let degree = trueHeading - Double(self.marker?.map?.camera.viewingAngle ?? 0)
            self.marker?.iconView?.transform = CGAffineTransform(rotationAngle: CGFloat(degree * Double.pi / 180))
