...
class MapView: MAMapView, MAMapViewDelegate {
    ...
    @objc func changeMapStyle() {
        ...
        mapType = isDark ? .standardNight : .standard
//        var mapStyle = "style.data"
//        if isDark {
//            mapStyle = "style_dark.data"
//        }
        
//        if mapStyle == self.mapStyle {
//            return
//        }
//        self.mapStyle = mapStyle
//        if let path = Bundle.currentBundle()?.path(forResource: mapStyle, ofType: nil) {
...
//        }
    ...
    func moveCamera(position: NSDictionary, duration: Int = 0) {
        ...
        var zoomLevel = (position["zoom"] as? Double)?.cgFloat ?? zoomLevel
...
//        }
        ...
        if let regionData = position["region"] as? NSDictionary {
            let latitudeDelta = regionData["latitudeDelta"] as? Double
            let longitudeDelta = regionData["longitudeDelta"] as? Double
            let span = MACoordinateSpan(latitudeDelta: latitudeDelta ?? 0, longitudeDelta: longitudeDelta ?? 0)
            let region = MACoordinateRegion(center: status.centerCoordinate, span: span)
